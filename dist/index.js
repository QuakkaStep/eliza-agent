...
// src/providers/poolInfo.ts
import {
  elizaLogger
} from "@elizaos/core";
import NodeCache from "node-cache";
var DEFAULT_POOL_ID = "GQsPr4RJk9AZkkfWHud7v4MtotcxhaYzZHdsPCg9vNvW";
var DYNAMIC_INFO_API = "http://localhost:3001/pool-monitoring/dynamic-info";
var DynamicPoolProviderService = class {
  cache;
  constructor() {
    this.cache = new NodeCache({ stdTTL: 60 });
  }
  async getDynamicPoolData(poolId) {
    elizaLogger.info(
...
// src/actions/generateConfig.ts
var IncreaseLiquidityConfigSchema = z.object({
  stepPercentage: z.number(),
  addLiquidityAmount: z.number(),
  minPrice: z.number(),
  maxPrice: z.number()
});
var generatePoolConfigTemplate = `
...
`;
var generateConfig_default = {
  name: "GENERATE_RAYDIUM_CLMM_CONFIG",
  similes: ["CREATE_RAYDIUM_CLMM_CONFIG", "BUIDING_POOL_CONFIG"],
  validate: async (_runtime, _message) => {
    return true;
  },
  description: "Create a Raydium CLMM pool config like token amount and step of Ladder-Step LP. Requires user token held by amount.",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.info("Starting generate raydium clmm config");
    elizaLogger2.info("Message: ", JSON.stringify(message.content, null, 2));
    if (!state) {
      elizaLogger2.info("No state provided, composing state");
      state = await runtime.composeState(message);
    } else {
      elizaLogger2.info("State provided, updating recent message state");
      state = await runtime.updateRecentMessageState(state);
    }
    state.currentMsg = message.content.text;
    const poolInfo = await poolProvider.get(runtime, message, state);
    elizaLogger2.info("Pool info: ", JSON.stringify(poolInfo, null, 2));
    state.poolInfo = poolInfo;
    const ctx = composeContext({
      state,
      template: generatePoolConfigTemplate
    });
    elizaLogger2.info("Template result: ", JSON.stringify(ctx, null, 2));
    ...
    const { stepPercentage, addLiquidityAmount, minPrice, maxPrice } = content.object;
    elizaLogger2.info("Generated config: ", content.object);
    if (callback) {
      callback({
...
// src/index.ts
var quokkaStepPlugin = {
  name: "quokkaStep",
  description: "quokkaStep plugin buiding on Raydium, As the ladder step and single-sided LP converte tool",
  actions: [generateConfig_default],
  evaluators: [],
  providers: [poolProvider]
};
var index_default = quokkaStepPlugin;
export {
